...
const CompanyLogo = () => {
  ...
  return (
    <div className='w-full overflow-hidden container mx-auto py-20 gap-8 flex sm:flex-row flex-col sm:items-center items-start'>
      <div className='w-[300px] shrink-0 text-gray-600 px-5 border-l-4 border-blue-500 bg-white py-2 z-10 sm:text-base text-xl font-semibold text-left '>
  ...
  {logos.map((logo,index)=>( 
            <img key={`dublicate-${index}`}src={logo} alt='company logo' className='mx-12 h-8 w-36 object-contain grayscale opacity-70 hover:grayscale-0 hover-opacity-100 transition-all'/>
    ))}
